...
class DistributedMapDemo:
    def __init__(self):
        ...
        # Using a blocking map proxy for put_all to ensure operations complete.
        # If you specifically need async operations elsewhere, you can get an async proxy separately.
        self.dist_map = self.client.get_map("capitals").blocking()

    def insert_entries(self): # Corrected indentation here
        print("Запишемо 1000 значень в Distributed Map ...")
        # Create a dictionary of all entries
        # Lab task: "з ключами від 0 до 1000" - this typically means 1001 items (keys 0, 1, ..., 1000).
        # If it means 1000 items starting from key 0, then range(1000) -> keys 0-999.
        # If it means 1000 items starting from key 1, then range(1, 1001) -> keys 1-1000.
        # Sticking to your original range(1, 1001) for keys 1-1000. Adjust if lab means 0-1000.
        entries_to_add = {str(i): f"City_{i}" for i in range(1, 1001)}

        try:
            # Using put_all for efficiency. Since self.dist_map is now blocking,
            # this operation will wait for completion.
            self.dist_map.put_all(entries_to_add)
            print(f"Значення ({len(entries_to_add)}) записані.")
            # Optional: Verify size after put_all
            # print(f"Розмір мапи 'capitals' після запису: {self.dist_map.size()}")
        except Exception as e:
            print(f"Помилка під час запису {len(entries_to_add)} значень через put_all: {e}")

    def shutdown(self): # Corrected indentation here (assuming it was also misaligned like insert_entries)
        print("Завершення роботи клієнта DistributedMapDemo...")
...
def main():
    ...
    # for inc_type, test_name in [ (2, "Песимистичная блокировка")]:
